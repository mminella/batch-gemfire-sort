 ...
 */
public class SortFileItemReader extends AbstractItemCountingItemStreamItemReader<Item> implements
		ResourceAwareItemReaderItemStream<Item>, InitializingBean {

	private static final Log logger = LogFactory.getLog(org.springframework.batch.item.file.FlatFileItemReader.class);

	// default encoding for inputStream files
	...
	private SortFileLineMapper lineMapper = new SortFileLineMapper();
	...
	public SortFileItemReader() {
		setName(ClassUtils.getShortName(SortFileItemReader.class));
	}

	/**
	 * In strict mode the fileChannel will throw an exception on
	 * {@link #open(org.springframework.batch.item.ExecutionContext)} if the inputStream resource does not exist.
	...
	/**
	 * Setter for the encoding for this inputStream source. Default value is {@link #DEFAULT_CHARSET}.
	 *
	 * @param encoding a properties object which possibly contains the encoding for this inputStream file;
	 */
	...
	/**
	 * Public setter for the inputStream resource.
	 */
	@Override
		...
		if (line == null) {
			...
			try {
				...
				throw new FlatFileParseException("Parsing error at line: " + lineCount + " in resource=["
						+ resource.getDescription() + "], inputStream=[" + new String(line) + "]", ex, new String(line), lineCount);
			}
		}
	}
	...
	private byte[] readLine() {
		...
		try {
			...
			noInput = true;
			throw new NonTransientFlatFileException("Unable to read from resource: [" + resource + "]", e, new String(line),
					lineCount);
		}
	...
	protected void doOpen() throws Exception {
		...
		if (!resource.exists()) {
			if (strict) {
				throw new IllegalStateException("Input resource must exist (fileChannel is in 'strict' mode): " + resource);
			}
		...
		if (!resource.isReadable()) {
			if (strict) {
				throw new IllegalStateException("Input resource must be readable (fileChannel is in 'strict' mode): "
						+ resource);
			}
		...
		inputStream = new DataInputStream(resource.getInputStream());
		noInput = false;
	}
